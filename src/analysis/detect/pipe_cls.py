...
class SplitKey(SplitPipe):
    def __init__(self, names=[], key="Maincategory") -> None:
        super().__init__()
        for name in names:
...
class SplitMaincategory(SplitKey):
    def __init__(self, label_list=["Overall","Bottom", "Top", "Outer"]) -> None:
        super().__init__(label_list, "Maincategory")
...
def test_split_pipe():
    print("========Run exe============")
    src = create_test_resource(metadata={"f_num": 1,"s":"frame(1)"})
    print("input")
    src.print()
    
    p_pipe = SplitMaincategory()
    ...
    src = create_test_resource(metadata={"f_num": 0,"s":"frame(0)"})
    print("input")
    src.print()
    ...
    for i in range(2, randrange(3,10)):
        src = create_test_resource(metadata={"f_num": i,"s":f"frame({i})"})
